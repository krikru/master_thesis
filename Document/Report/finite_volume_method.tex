...
In order for $\mathbf{p}^{(k)}$ to converge to the solution, $\mathbf{p}$, the error $\mathbf{\epsilon}^{(k)}$ must converge to the zero vector, so in turn, every eigenvalue $\lambda$ of $\mathbf{B}$ must satisfy the inequality $|\lambda| < 1$, otherwise some components of the error will never vanish, and might even grow in amplitude instead of decrease. If a component in the error has an eigenvalue whose absolute value is very close to one, that component will decrease very slowly.
