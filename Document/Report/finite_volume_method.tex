...
If we assume that $\mathbf{v}$ is an eigenvector of $\mathbf{B}$ with the eigenvalue $\lambda$, we find that 
%
\begin{equation}
...
If we for the moment assume that the grid is a $d$-dimensional, cubic --- or the corresponding for $d \neq 3$ --- grid with the grid spacing $\Delta r$ and with periodic boundaries, the simplest useful definition of $\nabla^2_{\text{D}}$ would be
%
\begin{equation}
\renewcommand*{\arraystretch}{2} 
\begin{array}{c}
\displaystyle \nabla^2_{\text{D}}\,\phi(\vec{r}) \,=\, \sum_{l=0}^{d-1} \frac{\phi(\vec{r}-\Delta r\,\normvec{e}_l)-2\,\phi(\vec{r})+\phi(\vec{r}+\Delta r\,\normvec{e}_l)}{\Delta r^2} \\
\displaystyle =\frac{\,-2\,d\,\phi(\vec{r})}{\Delta r^2}+\sum_{l=0}^{d-1} \frac{\phi(\vec{r}-\Delta r\,\normvec{e}_l)+\phi(\vec{r}+\Delta r\,\normvec{e}_l)}{\Delta r^2},
\end{array}
\end{equation}
%
where $\phi(\vec{r})$ is an arbitrary scalar or vector field and $\mathbf{e}_l$ is the $l$th orthonormal basis vector. If we use this definition of $\nabla^2_{\text{D}}$ as substitute for $\nabla^2$ when discretizing \eqref{eq:pressure_poisson_equation_continuous}, we obtain
%
\begin{equation} \label{eq:pressure_poisson_equation_discretized}
\frac{-2\,d\,p(\vec{r})}{\Delta r^2}+\sum_{l=0}^{d-1} \frac{p(\vec{r}-\Delta r\,\normvec{e}_l)+p(\vec{r}+\Delta r\,\normvec{e}_l)}{\Delta r^2} \,=\, q(\vec{r}).
\end{equation}
%
...
(\mathbf{D}\mathbf{p})_j \,=\, \frac{-2\,d\,p(\vec{r}_j)}{\Delta r^2} \,=\, \frac{-2\,d\,p_j}{\Delta r^2} \,=\, \left(\frac{-2\,d}{\Delta r^2}\mathbf{p}\right)_j
\end{equation}
%
and
%
\begin{equation} \label{eq:diagonal_matrix_structured_grid}
(\mathbf{R}\mathbf{p})_j \,=\, \sum_{l=0}^{d-1} \frac{p(\vec{r}_j-\Delta r\,\normvec{e}_l)+p(\vec{r}_j+\Delta r\,\normvec{e}_l)}{\Delta r^2},
\end{equation}
%
where $\vec{r}_j$ is the location of, and $p_j$ is the pressure in, the $j$th grid point, respectively. Note that in this particular case, $\mathbf{D}$ reduces to just a scalar ($-2\,d\,\Delta r^{-2}$) times the identity matrix, $\mathbf{I}$. For a non-structured grid, this is typically not the case.

\HRule

and that $\mathbf{v}$ is a vector on the form
%
\begin{equation} \label{eq:jacobi_regular_wave}
v_j = e^{i\mathbf{k}^{\T}\mathbf{r}_j},
\end{equation}
%
where $v_j$ is the $j$th element of $\mathbf{v}$, $\mathbf{k}$ is a wave vector associated with $\mathbf{v}$ and $\mathbf{r}_j$ is the position of the $j$th grid point, \eqref{eq:jacobian_fastest_convergence} can be rewritten as
%
\begin{equation}
a\sum_{l=0}^{d-1} e^{i\mathbf{k}^{\T}(\mathbf{r}_j-\Delta r\mathbf{e}_{l})} + e^{i\mathbf{k}^{\T}(\mathbf{r}_j+\Delta r\mathbf{e}_{l})} \,=\, 0,\ \forall j,
\end{equation}
%
where $a$ is a real number associated with $\mathbf{R}$ and $\mathbf{e}_l$ is the $l$th orthonormal basis vector. By dividing by $ae^{i\mathbf{k}^{\T}\mathbf{r}_j}$ at both sides, this system of linear equations reduces to just
%
\begin{equation}
\sum_{l=0}^{d-1} e^{-i\mathbf{k}^{\T}\Delta r\mathbf{e}_{l}} + e^{i\mathbf{k}^{\T}\Delta r\mathbf{e}_{l}} \,=\, 2\sum_{l=0}^{d-1} \cos(\mathbf{k}^{\T}\Delta r\mathbf{e}_{l}) \,=\, 0.
\end{equation}
%
On a three-dimensional grid, the solution of this equation will give a wavelength between $4/\sqrt{5}$ and $4/\sqrt{3}$ cell sizes. Any vector given by \eqref{eq:jacobi_regular_wave}, with a wavelength longer than $4/\sqrt{3}$ will have a positive eigenvalue, and any such eigenvector with a wavelength shorter than $4/\sqrt{5}$ will have a negative eigenvalue, so the wavelengths in $\mathbf{\epsilon}$ that decreases the quickest in strength are between $4/\sqrt{5}$ and $4/\sqrt{3}$ cell sizes.
