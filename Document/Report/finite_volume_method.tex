...
\begin{enumerate}
    \item Calculate the cell-centered velocity vectors by averaging the face-centered velocity scalars, tentatively using the area of the cell faces times the sum of the censities in the two adjacent cells for averaging (as done in this case). For simplicity, let's make the (incorrect) assumption that the velocity field $\vec{u}$ is location independent. We can use \eqref{eq:fi_flat_cell_face} since $\vec{u}$ is a vector field, substitute $\vec{u}$ for $\vec{F}$ and rewrite the equation as
    
    \begin{equation} \label{eq:u_scalar_i_from_u_vec}
    u_i \,=\, \normvec{n}_i\cdot\vec{u}.
    \end{equation}
    
    Let's define the \idxs{help}{variable} $\vec{u}^*$ as
    
    \begin{equation} \label{eq:u_vec_help_variable}
    \vec{u}^* \,=\, \sum_{S_i}w_i\,\normvec{n}_i\,u_i,
    \end{equation}
    
    where $S_i$ is the cell face between the cell $\vec{u}$ is being calculated for and the $i$th neighbor cell and $w_i$ is the weight of the velocity scalar at the cell face $S_i$ used in the averaging. In \thisprojectwork, $w_i$ has been defined as
    
    \begin{equation}
    w_i \,=\, S_i\,(\rho_0 + \rho_i),
    \end{equation}
    
    where $\rho_0$ is the average density of the cell $\vec{u}$ is being calculated for and $\rho_i$ is the average density of the $i$th neighbor cell. By using \eqref{eq:u_scalar_i_from_u_vec} in \eqref{eq:u_vec_help_variable}, we obtain
    
    \begin{equation}
    \vec{u}^* \,=\, \sum_{S_i}w_i\,\normvec{n}_i\,\normvec{n}_i\cdot\vec{u}.
    \end{equation}
    
    By realizing that $\sum_{S_i}w_i\,\normvec{n}_i\,\normvec{n}_i$ can be considered to be a matrix, which is in fact invertible, we can multiply both sides of the equation with the inverse and obtain
    
    \begin{equation}
    \left(\sum_{S_i}w_i\,\normvec{n}_i\,\normvec{n}_i\right)^{-1}\vec{u}^* \,=\, \vec{u}.
    \end{equation}
    
    By using \eqref{eq:u_vec_help_variable}, this turns into
    
    \begin{equation}
    \vec{u} \,=\, \left(\sum_{S_i}w_i\,\normvec{n}_i\,\normvec{n}_i\right)^{-1}\sum_{S_i}w_i\,\normvec{n}\,u_i,
    \end{equation}
    
    which can be used to calculate the cell-centered velocity vectors.
    
    In \thisprojectwork, the cells are \idxe{axis alignment}{axis-aligned}, so $\sum_{S_i}w_i\,\normvec{n}_i\,\normvec{n}_i$ will be a diagonal matrix and the inverse will just be the matrix consisting of the inverse of the diagonal elements. In fact, $\vec{u}$ can be calculated simply by calculating the weighted average of all $\normvec{n}_i\,u_i$ for which $\normvec{n}_i\parallel \normvec{e}_j$, where $\normvec{e}_j$ is the basis vector belonging to the $j$th dimension, such that $(\normvec{e}_0,\,\normvec{e}_1,\,\normvec{e}_2) = (\normvec{x},\,\normvec{y},\,\normvec{z})$, doing so for $j = 0,\, 1,\, 2$ and adding the three averages together, that is
    
    \begin{equation}
    \vec{u} \,=\, \sum_{j=0}^2 \frac{|\normvec{n}_i\cdot\normvec{e}_j|\,w_i\,\normvec{n}_i\,u_i}{|\normvec{n}_i\cdot\normvec{e}_j|\,w_i}.
    \end{equation}
    
    \item Calculate the face-centered \idxsp{velocity}{vector}{s} from the cell-centered velocity vectors, by using a bounded advection scheme since these are the vectors that are going to be advected. In \thisprojectwork, the \UPWIND scheme has been used, for which this step is simply a means of picking the cell-centered velocity vector in the cell the flow is comming from and assigning that to the cell face.
    ...
    \item Since the momentum in one cell, created by the velocity scalars in the cell face of the cell, automatically changes when the mass in the cell changes, calculate how much of the net momentum inflow it is that is actually created by the mass inflow. Use only the part of the cell face control volumes that lie within the cell when doing so and consider the mass to have been distributed homogenously within the cell. Note that together these parts cover exactly the entire cell once in each dimension. Since this part of the net momentum inflow has effectively already heen added to the cell, subtract it from the net momentum inflow to obtain the part of the net momentum inflow that still has to be distributed on the cell faces.
    
    \item Distribute the part of the net momentum inflow, that still has to be distributed, on the cell faces as momentum-scalars, by using the mass of the cell face control volume, only of the part that lie within the cell, as weight. Note that this is equivalent to using the volume of the part of the cell face control volume that lies within the cell as weight, since mass was considered to be distributed homogenously within the cell. In \thisprojectwork, since the cells are cubic and the control volume extends through half the cell, the volume of the part of the cell face control volume that lies within the cell is proportional to the cell face area, so in this case the weight is simply the cell face area.
    
    Since we have distributed a momentum on velocities, we have 
    
    \item Calculate the updated face-centered velocity scalars by multiplying the face-centered momentum scalars with the total mass contained withing the control volume of the cell face the velocity scalar is stored on.
\end{enumerate}

This method also conserves momentum.
