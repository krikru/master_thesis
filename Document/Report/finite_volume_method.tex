...
where $\vec{u}_{n}$ denotes the velocity in time step $n$. Using a method described e.g.\ in \citep{Losasso2004}, this \PDE can be solved in two steps. First, an auxiliary velocity $\vec{u}^*_n$ is calculated ignoring the pressure term, that is
...
Furthermore, it turns out that when the flow is compressible, we have

\begin{equation} \label{eq:deviatoric_stress_tensor_incompressible_flow}
\nabla\cdot\boldsymbol{\mathsf{T}} \,=\, \mu\nabla^2\vec{u},
\end{equation}

where $\mu$ is the (dynamic) \viscosity\index{dynamic viscosity|see{viscosity}}. For simplicity, we can assume that we use a set of units where $\rho = 1$. \eqref{eq:auxiliary_velocity} can then be rewritten as
