...
\end{equation}
%
where $\mathbf{B} = -\mathbf{D}^{-1}\mathbf{R}$, and this in turn leads to
%
\begin{equation} \label{eq:jacobi_method_error_from_initial_error}
\mathbf{\epsilon}^{(k)} \,=\, (\mathbf{B})^k\mathbf{\epsilon}^{(0)},
\end{equation}
%
which tells us that the error will decrease exponentially to the number of iterations. The error can be seen as a linear combination of the \eigenvectors of $\mathbf{B}$, which will decrease with different speeds. In fact, an eigenvector of $\mathbf{B}$ will represent a wave shape, and can be assigned an approximate \wavelength in number of cells.

The components in the error that decreases the quickest are the eigenvectors with the corresponding eigenvalues closest to 0. If we assume that $\mathbf{v}$ is an eigenvector with the eigenvalue 0, we find that 
%
\begin{equation}
-\mathbf{D}^{-1}\mathbf{R}\mathbf{v} \,=\, 0\mathbf{v} \,=\, \mathbf{o},
\end{equation}
%
where $\mathbf{o}$ is the zero vector, and by multiplying both sides with $-\mathbf{D}$ from the left, we obtain
%
\begin{equation} \label{eq:jacobian_fastest_convergence}
\mathbf{R}\mathbf{v} \,=\, \mathbf{o}.
\end{equation}
%
If we also assume that the grid is a $d$-dimensional, cubic --- or the corresponding if $d \neq 3$ --- grid with the grid spacing $\Delta r$ and with periodic boundaries, and that $\mathbf{v}$ is a vector on the form
%
