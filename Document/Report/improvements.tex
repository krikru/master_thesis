...
Typically, when rendering an \idxs{ocean}{scene}{s}, not the entire wave spectrum is used, but only the lowest frequencies of it, since too high frequencies will give rise to \folding, which may in turn give rise to \idxsp{Moire}{pattern}{s}. Therefore, not all frequencies will be caught in \idxs{rendered}{image}. This is especially notable if much of the \idxs{wave}{energy} in a \idxs{sea}{state} lies in the part of the \idxs{wave}{spectrum} where the \wavelength is shorter than the shortest wavelength that can be rendered correctly, without introducing folding. Besides, because of the limited resolution of the \grid, wavelengths that are too short are automatically removed even if the rendering would allow shorter wavelengths.
...
If we simplify a water surface by removing a part of the \idxs{wave}{spectra}, we need to -- for \rendering purposes -- compensate for that by making the \reflection in the \idxs{simplified}{water surface} \idxse{diffuse}{reflection}{diffuse}.
...
%This means that there is in practice many different directions the light can \bounce off in after having hit a simplified surface, whilst for a \idxs{non-simplified}{water surface}, the light can only bounce off in one direction, which is given by the \idxs{Householder}{transformation}.
