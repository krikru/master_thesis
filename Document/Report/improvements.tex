...
In order to obtain a realistic \idxs{sea}{state}\index{state of sea|see{sea state}} in a simulation, it is desirable that the \idxs{water}{surface} \interacts with the \wind in order to give rise to \idxsp{wind}{wave}{s}, which makes the sea state a reflection of the wind speed and direction.

\subsubsection{Spectral methods}

A very cheap and easy way to simulate wind waves is to use a wind dependent \idxs{wave}{spectrum}. A wave spectrum tells how the size of waves with different wavelengths are distributed, and from those distributions the \idxs{Fourier}{transform}.

 how large waves with different wavelength are.

, which basically tells how much energy that is stored in each part of the wave spectrum in \average, depending on the \idxs{wind}{velocity}. The wave spectrum $P_h(\vec{k})$ is defined as
