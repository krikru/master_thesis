...
For a \idxs{uniform}{surface grid}, the convolution filter can be applied by using the a modified version of the \FMM \citep{Greengard1985,Greengard1987a} for continuous data, the \CFMM \citep{White1994}, which allows it to be applied with a $O(n)$\indexs{big O}{notation} \idxe{amortized running time}{amortized} \idx{time complexity} instead of $O(n^2)$ as with a simple $n$-to-$n$ approach, where $n$ is the number of surface grid points.
