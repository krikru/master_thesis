...
For a \idx{uniform surface grid}\index{surface grid!uniform}, the convolution filter given by \eqref{eq:opc} can be applied by using a variant of the \idx{Fast Multipole Method} (\qidxe{FMM}{FMM|see{Fast Multipole Method}}) \citep{Rokhlin1985,Greengard1987} for continuous data, called the \idx{Continuous Fast Multipole Method} (\qidxe{CFMM}{CFMM|see{Continuous Fast Multipole Method}}) \citep{White1994,White1996}, which will allow it to be applied in $O(1)$\index{big O notation} time in average per surface grid point.

However, even though $O(1)$ is very fast in theory, the Fast Multipole Method is complicated and involves many computational steps, which would make the simulation slow. It needs many \idxe{Taylor terms}{Taylor term} to calculate a good approximantion, so one would have balance the approximation errors with the additional computational costs due to the processing the Taylor terms.
