...
\begin{figure}
    %\centering
    \subcaptionbox{\label{fig:tranasformedkernelcomparison}}[.45\textwidth]{\input{comparison.tex}}
    \subcaptionbox{\label{fig:tranasformedkernelratio}}[.45\textwidth]{\input{ratio.tex}}
    \caption{Comparisons between the signal response of the kernel given by \eqref{eq:empirical} and the signal response of an ideal kernel. The fluctuations in \subrefp{fig:tranasformedkernelratio} for the higher values of $\xi$ are numerical errors introduced when generating the curve.}
