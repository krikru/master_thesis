...
However, this equation contains a second unknown, $p$, which must be calculated before $\vec{u}$ can be calculated. By taking the divergence of both sides of \eqref{eq:velocity_update} and using \eqref{eq:velocity_divergence_incompressible_flow} to get rid of $\nabla\cdot\vec{u}$, we obtain the \idxs{Poisson}{equation}
