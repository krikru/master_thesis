...
In \thiswork, an octree has been used to partition the space into the \cells required by the \FVM. Since only the \surface of the water is visible in the \simulation, the \idxsp{surface}{cell}{s} have been given a high \LOD; the \LOD then decreases at the water depth increases, staying at each \LOD a few \idxse{layer of}{cells}{layers of cells} at a time. Ideally, the \LOD on the surface will also dependent on the horizontal location and be higher where it is more important to catch much of the \idxsp{surface}{detail}{s}, such as close to the \camera. The total number of cells $N_{\text{t}}$ used in the simulationcan can be approximated by an expression containing the thicknesses of the different \LODs and the number of cells visible at the surface that belong to each \LOD accoring to
