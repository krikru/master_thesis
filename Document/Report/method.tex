...
In \thiswork, an octree has been used to partition the space into the \cells required by the \FVM. Since only the \surface is visible in the \simulation, the \idxsp{surface}{cell}{s} have been given a high \LOD; the \LOD then decreases periodically, a few \idxse{layer of}{cells}{layers of cells} further down under the surface. This means that the total number of cells used in the simulation, $N_{\text{total}}$, is limited by a \idxs{geometric}{sum} multiplied the number of cells visible on the surface, $N_{\text{surface}}$, that is

\begin{equation} \label{eq:number_of_cells_total}
N_{\text{total}} \,=\, 1\,$N_{\text{surface}}$
\end{equation}

%An \octree is a spatial tree \idxs{data}{structure} in which each \node corresponds to a \cube in the \space, and where each cube corresponding to an \idxs{internal}{node} is subdivided into eight half-sized \idxsp{child}{cube}{s}. The 
