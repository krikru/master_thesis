...
Here, we can identify the \idxs{vector}{triple product}\index{product!triple, vector|see{vector triple product}}, which can be rewritten as

\begin{equation}
(\vec{a}\times \vec{b})\times \vec{c} \,=\, -\vec{c}\times(\vec{a}\times \vec{b}) \,=\, -(\vec{c}\cdot\vec{b})\,\vec{a} + (\vec{c}\cdot\vec{a})\,\vec{b}.
\end{equation}

By using this substitution, \eqref{eq:domega_dh} becomes
