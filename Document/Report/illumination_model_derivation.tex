...
{
If we simplify a water surface by cutting off a part of the \idxs{wave}{spectrum}, such as the part containing all frequencies over the \idxs{Nyquist}{frequency}, we will loose details on the surface which we --- for \visualization purposes --- need to compensate for by making \reflections in the water surface \idxse{diffuse}{reflection}{diffuse}.
...
\begin{equation} \label{eq:domega_dh}
\renewcommand*{\arraystretch}{2}
\begin{array}{l}
...
By using this identity, \eqref{eq:domega_dh} becomes
...
By using this result and \eqref{eq:normal_from_anti_normal}, we can rewrite \eqref{eq:rendering_equation_reduced_rhor_rhot} as

\begin{equation} \label{eq:rendering_equation_reduced_rhot}
\renewcommand*{\arraystretch}{1.5}
\begin{array}{cl}
L_{\text{o}}(\normvec{\omega}_{\text{o}}) \,= \\
\displaystyle \int_{\Omega_{\text{r}}} \frac{\anormvec{h}^4}{4\,\normvec{\omega}_{\text{o}}\cdot\anormvec{n}}\,f(\nabla\eta^*(\vec{r}))\,R_{\normvec{n}^*\!,x}(\normvec{\omega}_{\text{o}})\, L_{\text{i}}(\normvec{\omega}_{\text{i}})\, \opd \normvec{\omega}_{\text{i}} \ + \\[1ex]
\displaystyle \int_{\Omega_{\text{t}}} \rho'_{\text{t}}(\normvec{\omega}_{\text{i}},\, \normvec{\omega}_{\text{o}})\, L_{\text{i}}(\normvec{\omega}_{\text{i}})\, (\normvec{\omega}_{\text{i}}\,\cdot\,(-\normvec{n}))\, \opd \normvec{\omega}_{\text{i}}\,.
\end{array}
\end{equation}
...
For a \idxs{point}{light source} (a light source) with the total \idxs{spectral}{radiant intensity}\index{intensity!spectral radiant|see{spectral radiant intensity}} (power per unit solid angle and unit wavelength) $I$, located at the distance $d$ in the direction $\normvec{\omega}_{\text{s}}$ above the water surface, the total spectral radiance directed outward along direction $\normvec{\omega}_{\text{o}}$ will be

\begin{equation} \label{eq:spectral_radience_reflected_point_light_source}
\renewcommand*{\arraystretch}{2}
\begin{aligned}
\displaystyle L_{\text{o}}(\normvec{\omega}_{\text{o}}) \,=\, & \int_{\Omega_{\text{r}}} \frac{\anormvec{h}^4}{4\,\normvec{\omega}_{\text{o}}\cdot\anormvec{n}}\,f(\nabla\eta^*(\vec{r}))\,R_{\normvec{n}^*\!,x}(\normvec{\omega}_{\text{o}})\,\frac{\delta_{\normvec{\omega}_{\text{i}},\normvec{\omega}_{\text{s}}}\,I}{d^2}\, \opd \normvec{\omega}_{\text{i}}. \\
\displaystyle =\, & \frac{\anormvec{h}^4}{4\,\normvec{\omega}_{\text{o}}\cdot\anormvec{n}}\,f(\nabla\eta^*(\vec{r}))\,R_{\normvec{n}^*\!,x}(\normvec{\omega}_{\text{o}})\,\frac{I}{d^2},
\end{aligned}
\end{equation}

where the restriction $\normvec{\omega}_{\text{i}} = \normvec{\omega}_{\text{s}}$ is made in order to calculate $\anormvec{h}$ and $\nabla\eta^*(\vec{r})$ in the last expression.