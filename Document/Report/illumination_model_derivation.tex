...
Let's assume that $\eta$ is periodic in both dimensions with the period $P$ (as often is the case in \idxs{computer}{graphics}), with a large surface element $A$ that keeps repeating itself, such that

\begin{equation} \label{eq:water_surface_periodicity}
\eta(\vec{r}+P\normvec{x}) \,=\, \eta(\vec{r}+P\normvec{y}) \,=\, \eta(\vec{r}),\,\forall\,\vec{r},
\end{equation}

where $\normvec{x}$ and $\normvec{y}$ are the orthonormal basis vectors generating the horizontal plane. This implies implies that $\fdfunc{\eta}$ is discrete with \idxsp{Dirac}{impulse}{s} lying at the fixed distance $d = 2\pi/P$ from each other, such that
