...
where $\fdfunc{\eta}(m,\,n)$ is a \twodimensional \sequence of coefficients associated with $\fdfunc{\eta}$, and $m$ and $n$ are two integers used to index the terms in the \series.
...
\begin{equation} \label{eq:fd_eta_one_dirac}
...
The correspondence of a normal distribution for vectors is called \textit{\href{http://en.wikipedia.org/wiki/Multivariate\_normal\_distribution\#Non-degenerate\_case}{Multivariate normal distribution}}, and requires that the \idxs{covariance}{matrix} of the vector is known. The covariance matrix, $\Sigma(\vec{x})$, of a stochastic vector $\vec{x}$ is defined as
