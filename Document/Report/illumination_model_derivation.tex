...
where an apostrophe denotes that the gradients of the free surface elevation for the included microfacets is restricted. When projected onto the plane orthogonal to $\normvec{\omega}_{\text{i}}$, the total area of all microfacets will be
...
So, for radiation coming from the direction $\normvec{\omega}_{\text{i}}$, with the \idxs{incident}{radient flux} $\opd \Phi_{\text{i}}(\normvec{\omega}_{\text{i}})$, the \idxs{reflected}{radient flux} that was reflected on a \microfacet with a free surface elevation whose gradient lies withing $\opd\nabla\eta^*$ --- $\opd^2 \Phi_{\text{o}}(\normvec{\omega}_{\text{i}})$ --- will be
...
By realizing that the normal of the a \microfacet, $\normvec{n}^*$, needs to equal $\normvec{h}$ in order for a ray incoming from the direction $\normvec{\omega}_{\text{i}}$ to be reflected in the direction $\normvec{\omega}_{\text{o}}$, we can use \eqref{eq:eta_anti_normal} to conclude that 

\begin{equation}
\opd \anormvec{h} \,=\, \opd\nabla\eta^*
\end{equation}

(note that most --- if not all --- of the infinitesimal quantities used in this appendix are two-dimensional in their nature; in this case the minus sign in \eqref{eq:eta_anti_normal} gets squared and vanishes). By combining this with \eqref{eq:gradient_restricted_reflected_flux_pre_incident_flux_per_unit_gradient_squared}, we can rewrite  \eqref{eq:brdf_from_ratio_between_fluxes}
