...
A set of equations that are better suited under these circumstances are the \idxs{shallow water}{equations}, even though these still don't catch the correct wave dispersion which makes them unable to simulate the Kelvin wake pattern. They are usually used to simulate waves whose \wavelength is similar to or greater than the overall \idxs{water}{height} \citep{Thurey2006}. A modified version of this set of equation was used by \citet{Kass1990} to do \SWS in computer graphics for the first time. \SWS can be used to simulate water flows such as dam breaks \citep{Brodtkorb2010,Brodtkorba2012}, storm surges and other types of floods, as well as tsunamis, but also non-water flows such as atmospheric flow and avalanches, as noted by \citet{Brodtkorb2011}.
...
We have the \MAC method, which is a \FVM simulation in which many small, massless marker particles are initially homogeneously distributed in the fluid and then carried with the flow to mark the presence of fluid, which is an alternative to the \idxs{volume of fluid}{method}.
