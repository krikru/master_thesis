...
where $C_{ij},\,j=0,\dots,n_i$ are a sequence of polynomial coefficients used in the \PDE for describing $\eta_i$, and $j$ is an index used to specify one of those coefficients.
...
The aim of this method, is the same as the aim of the restricted tall cell grid approach, to reduce the number of cells that are needed in the simulation. It does so by modeling the grid with an \octree which allows for easy \idxs{adaptive}{mesh refinement}. This method was probably first implemented in \citeyear{Popinet2003} \citep{Popinet2003}, but has since been implemented a number of times after that \citep[e.g][]{Losasso2004,openfoam}.
