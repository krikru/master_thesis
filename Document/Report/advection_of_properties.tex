...
The choise of \idxs{advection}{scheme} depends entirely on the type of filed that is being exposed to advection and the necessary requirements that field puts on the advection scheme.
...
\begin{equation}
\begin{array}{c}
A\,\Phi_2 \,=\, A\,(\Phi \,-\, \tfrac{1}{2}(\Phi_1 \,-\, \Phi)) \,=\, A\,\Phi \,-\, \tfrac{1}{2}(A\,A'A\,\Phi \,-\, A\,\Phi) \\
=\, A\,(\tfrac{3}{2} \,-\, \tfrac{1}{2}A'A)\,\Phi \,=\, \tilde{A}\,\Phi,
\end{array}
\end{equation}
...
\tilde{A} \,=\, A\,(\tfrac{3}{2} \,-\, \tfrac{1}{2}A'A)
\end{equation}
...
\begin{equation}
\tilde{A}\,\phi \,=\, A\,(\tfrac{3}{2} \,-\, \tfrac{1}{2}A'A)\,\phi \,=\, \lambda\,(\tfrac{3}{2} \,-\, \tfrac{1}{2}\overline{\lambda}\lambda)\,\phi \,=\, \frac{\lambda}{|\lambda|}\,(\tfrac{3}{2}\,|\lambda| \,-\, \tfrac{1}{2}|\lambda|^3)\,\phi \,=\, \tilde{\lambda}\,\phi,
\end{equation}

where $\tilde{\lambda} = \lambda/|\lambda|\,(\tfrac{3}{2}\,|\lambda| \,-\, \tfrac{1}{2}|\lambda|^3)$ is the eigenvalue of $\phi$ when $\tilde{A}$ is acting on it. If $|\lambda|$ is written as
...
The damping introduced by $\tilde{A}$ is given by $\tilde{h} = 1 - |\tilde{\lambda}|$. If we assume that possible instabilities in $A$ are small, which means that $-h \ll 1$ if $h < 0$, and since $h \leq 1$, $\tilde{h}$ can be expanded to
...
\idxse{geometric}{advection scheme}{Geometric advection schemes} work by constructing a geometry for the surface by converting the discretized \idxs{phase fraction}{field} to a non-discretized phase fraction field, and by using the continuous field to calculate the fluxes between each ajacent pair of cells.
