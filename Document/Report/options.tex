...
\newflag{REVERTINDEXORDEROFSPLITKEYS}{true}  % Controls which index key refers to the other one for split keys
...
\newrobustcmd{\qindexify}[1]{#1} % For marking indexed text quietly (don't do anything)
...
\declareacronym {BEM} {boundary element method}
\declareacronyms{CFD} {computational}{fluid dynamics}
%\declareacronym {CFL} {Courant--Friedrichs--Lewy}
...
\declareindexkey      {estimation}
\declareindexkeypair      {estimation}{estimate}
...
\declareindexkey      {incompressible}
